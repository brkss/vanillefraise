...
let ActivityOverviewResolver = class ActivityOverviewResolver {
    async getUserBurnedCalories(ctx) {
        ...
        const { sum } = await (0, typeorm_1.getRepository)(Activity_1.Activity).createQueryBuilder('activities').select("SUM(calories)", 'sum').where('userID = :userid').setParameters({ userid: user.id }).getRawOne();
        return sum;
    }
};
__decorate([
